...
mysql = MySQL()
app.config['MYSQL_DATABASE_HOST']='jairmorocho1999.mysql.pythonanywhere-services.com'
app.config['MYSQL_DATABASE_USER']='jairmorocho1999'
app.config['MYSQL_DATABASE_PASSWORD']='jOsuEjUan'
app.config['MYSQL_DATABASE_DB']='jairmorocho1999$default'
mysql.init_app(app)
...
def index():
    ...
    try:
       ...
       if tipo==1:
            ...
            sql+= "AND NOT tipo_usuario.ID_TIPO=4;"
            conn= mysql.connect()
            cursor= conn.cursor()
            cursor.execute(sql)
...
##cambio de clave por un usuario que se encuentre en sesion
...
################Parte farmaceutico
#Ver lista de solicitudes por registrar
...
def verPeticionF(id):
    try:
        ...
        if tipo == 3:
            ...
            #consultar cabecera de la solicitud
            ...
            #consultar productos medicos de la lista
